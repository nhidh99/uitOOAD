...
public class NhanVienDAO {
		...
		try {
			...
			while (rs.next()) {
				output.add(new NhanVienDTO(rs.getInt("MaNhanVien"), rs.getString("TenNhanVien"), rs.getString("CMND"),
						...
						rs.getString("ChucVu"), rs.getString("TenTaiKhoan"), rs.getString("MatKhau")));
			}
			return output;
		} finally {
			conn.close();
	...
	public static NhanVienDTO getNhanVienById(Integer id) throws SQLException {
		...
		try {
			...
			ResultSet rs = statement.executeQuery(query);
			rs.next();
			NhanVienDTO output = new NhanVienDTO(rs.getInt("MaNhanVien"), rs.getString("TenNhanVien"),
					rs.getString("CMND"), rs.getString("DiaChi"), rs.getString("Email"), rs.getString("SoDienThoai"),
					rs.getString("ChucVu"), rs.getString("TenTaiKhoan"), rs.getString("MatKhau"));
			return output;
		} finally {
			conn.close();
