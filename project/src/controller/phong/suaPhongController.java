...
public class suaPhongController implements Initializable{
	...
	public void handleBtnXacNhan(ActionEvent e) throws SQLException {
		try {
			...
			if (PhongBUS.updatePhong(phongCu,phongMoi)) {
				Alert alert = new Alert(AlertType.INFORMATION);
				alert.setTitle("Thành công");
				alert.setHeaderText("Sửa phòng thành công!");
				alert.setContentText("Phòng " + phongMoi.getMaPhong() + " đã được sửa thành công!");
				alert.showAndWait();
				...
				alert.setContentText("Số phòng "+ phongMoi.getMaPhong() + " đã tồn tại, vui lòng nhập số phòng khác!");
				alert.showAndWait();
			}
	...
	public void initValue(PhongDTO phongCu) {
		this.phongCu = phongCu;
		tfMaPhong.setText(phongCu.getMaPhong());
		tfGhiChu.setText(phongCu.getGhiChu());
		
		ObservableList<LoaiPhongDTO> dsLoaiPhong = cbbLoaiPhong.getItems();
		for(int i = 0 ; i < dsLoaiPhong.size(); i++) {
			if(phongCu.getLoaiPhong().getMaLoaiPhong().equals(dsLoaiPhong.get(i).getMaLoaiPhong())) {
		...
		for(int i = 0 ; i < dsTinhTrang.size(); i++) {
			if(phongCu.getTinhTrang().getMaTinhTrang().equals(dsTinhTrang.get(i).getMaTinhTrang())) {
