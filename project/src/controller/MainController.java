...
public class MainController implements Initializable {
	...
	public void handleLapPhieuThue(ActionEvent e) {
		if (ConfirmDialogHelper.confirm("Bạn đã chắc chắn các thông tin để lập phiếu?")) {
			try {
				if(tfPT_KhachThue.getText().matches("^[a-zA-Z]") || !tfPT_CMND.getText().matches("^[0-9]{1,15}") || !tfPT_DienThoai.getText().matches("^[0-9]{11}")) {
					Alert alert = new Alert(AlertType.INFORMATION);
					alert.setTitle("Thất bại");	
					alert.setHeaderText("Lập phiếu thuê thất bại!");
					alert.setContentText("Tên khách chỉ gồm chữ cái viết hoa và viết thường! CMND chỉ gồm chữ số và có tối đa 15 kí tự! Số điện thoại chỉ có tối đa 11 chữ số!");
					alert.showAndWait();
				}
	...
	public void handleChotThongTinKhach(ActionEvent e) {
		if(tfHD_KhachTra.getText().matches("^[0-9]$") || !tfHD_CMND.getText().matches("^[0-9]{1,15}$") || !tfHD_DienThoai.getText().matches("^[0-9]{11}$")
				|| !tfHD_TienNhan.getText().matches("^[0-9]$")) {
			Alert alert = new Alert(AlertType.INFORMATION);
			alert.setTitle("Thất bại");	
